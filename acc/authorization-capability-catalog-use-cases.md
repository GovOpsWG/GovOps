...
4. [UC-03: Lint tool](#uc-03-lint-tool)
...
| Lending Officer            | Loan approval policy and lending capability stewardship | —                   | UC-02                 |
| Policy Engine Operator     | PDP policy lifecycle                                    | —                   | UC-04                 |
...
`govops lint` is the day-to-day validator for `#AuthorizationCapabilityCatalog` catalogs.
This use case shows how a Platform Security Engineer uses the tool to catch profile,
identity, and lexicon errors before a catalog change is accepted — independent of
policy drift checks (UC-04) or CI wiring.
...
The engineer stages a catalog change that breaks three independent rules: an incorrect
capability id, an out-of-enum `risk-tier`, and a missing `action` field (required by the
...
Each error names the capability id (when present), the rule violated, and a remediation
hint. Exit code `1` signals that tooling or a reviewer must reject the change.

**Step 4 — Fix and re-validate.**

The engineer restores the correct id for the transfer capability, sets
`risk-tier: critical` on the loan capability, and either completes or removes the
incomplete fraud entry. Re-running lint returns EXIT 0 with four valid capabilities
(the UC-01 base catalog).
...
| Artifact           | Gemara type                       | Path                     | Notes                                                         |
| ------------------ | --------------------------------- | ------------------------ | ------------------------------------------------------------- |
| Capability catalog | `#AuthorizationCapabilityCatalog` | `govops/GovOps-ACC.yaml` | Temporarily edited for lint exercises; restored to UC-01 base |
...
| `govops lint` tool               | Design §10  |
| Authorization Capability Profile | Design §6.2 |
| Capability id convention         | Design §6.2 |
...
divergence across published Cedar policy releases (payments and fraud), how the engineer decides
how to resolve each finding, and how ungoverned policy changes are caught before they reach production.
...
The fraud team added an escalation capability to the fraud Cedar policy without going through
...
The unified drift summary at the end of the `govops drift` output composes per-release
sub-reports into a single view. Each finding is tagged with its policy artifact so the
...
The per-release sub-reports are also available individually for service-specific
